...
from anvn_model_utils import AnvnPreModel, AnvnDataset, AnvnModelRun
...
class AnvnWidget(QMainWindow):
    ...
    def add_visualization_widget(self, table):
        ...
        self.data_operation_widget.set_callback_func(data_visualization_widget.data_monitor)
    ...
    def dataset_run_func(self, index):
        if index == AnvnDataset.success:
            ...
            self.data_operation_widget.set_data_load_value(100)
            self.data_operation_widget.set_data_load_text('Data processing completed:')
            self.model_run.start()
        else:
            self.data_operation_widget.set_data_load_value(int(index / len(self.dataset) * 100))
            
    
    def model_run_func(self, index):
        if index == AnvnModelRun.success:
            self.data_operation_widget.set_tokenizer(self.pre_model.get_tokenizer()).set_outputs(self.model_run.get_outputs()).set_all_ots(self.model_run.get_all_ots()).sel_all_iis(self.model_run.get_all_iis())
            self.data_operation_widget.set_model_run_value(100)
            self.data_operation_widget.set_model_load_text('Model running completed:')
            self.data_input_widget.set_run_disabled(False, '')
            self.data_operation_widget.disabled(False)
        else:
            self.data_operation_widget.set_model_run_value(int(index / len(self.model_run) * 100))

    def pre_model_init_func(self):
        self.data_input_widget.set_run_disabled(False, '')
        self.data_operation_widget.set_model_load_text('Model loading completed:')
        self.data_operation_widget.model_load_stop()
